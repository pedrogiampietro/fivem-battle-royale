...
export const addPlayerToMatchmaking = async (
  request: Request,
  response: Response
) => {
  try {
    ...
    if (
      ...
      io.emit("match-ready", { gameType, matchId: match.id });
...
// Função auxiliar para retornar o número de jogadores necessário para iniciar o jogo
function getRequiredPlayers(gameType: GameType): number {
  switch (gameType) {
    case GameType.SOLO:
      return 1;
    case GameType.DUO:
